...
``base`` is intentionally dropped (``get_base_ptr`` had no callers) and
``rank_idx`` lives outside the offset table: ``_SymBufferHostAdapter``
marshals it as a separate i32 scalar and the kernel consumes it as the device
``local_rank`` (see ``megamoe_kernel.py``). Freeing those struct slots lets
all 16 byval lanes hold offsets (EP16, not EP14).
"""
...
# TODO: Remove once the compiler is fixed. Workaround for a cuda-to-llvm bug:
# any kernel arg marked `grid_constant + byval` is treated as a tma_desc.
...
class SymBufferDeviceBase:
    ...
    @cute.jit
    def ptr_map_to_rank(
        self, ptr, dst_rank_idx: Int32, byte_align: Optional[int] = None
    ):
...
class SymBufferHost:
    ...
    Marshalled across the JIT boundary by ``_SymBufferHostAdapter`` (registered
    below), which re-wraps the scalar fields with ``Int64(...)``."""
...
class _SymBufferHostAdapter:
    ...
    offsets are i64, rank_idx is i32, and num_max_ranks remains a
