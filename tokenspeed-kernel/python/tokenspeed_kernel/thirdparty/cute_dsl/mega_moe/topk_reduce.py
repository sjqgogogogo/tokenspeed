...
# lowers to an ALU subnormal-normalization path (~60% DRAM SOL). The helper
# below forces a table-driven decode instead; ``e2m1_reg`` (N e2m1 codes, N % 8
# == 0) is read as packed b32 words and N fp32 values are written into
# ``fp32_reg`` in code order. The 16 e2m1 values are exact in fp32.
# ---------------------------------------------------------------------------
...
class TopkReduce:
    ...
    torch->cute conversion and the ``cute.compile``.
    """
    ...
    def _reduce_mxfp8(
        ...
        if (not needs_guard) or token_idx < reduced_output.shape[0]:
            ...
            for k in cutlass.range_constexpr(0, num_topk, 1):
                term = cute.make_rmem_tensor((hidden_per_thread,), cutlass.Float8E4M3FN)
