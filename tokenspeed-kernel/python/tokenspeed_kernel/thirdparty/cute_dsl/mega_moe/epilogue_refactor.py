...
from cutlass.cutlass_dsl import Float32, Int64, T
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.contract import (
    Contract,
    FunctionMapping,
    Space,
    eval_function_mapping,
)
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.fc1_fc2_fuse_sched import BlockPhase
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.flag_batch import (
    GpuReleaseFlagBatchTracker,
)
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.iket_compat import iket
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.megamoe_constants import (
    Fp8E4M3RcpLimit,
    Fp32Max,
    Nvfp4BlockSize,
    Nvfp4E2M1RcpLimit,
)
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.moe_persistent_scheduler import (
    MoESchedConsumer,
    MoESchedExtension,
    MoEWorkTileInfo,
)
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.ptx_helpers import (
...
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.sym_buffer import (
    SymBufferDeviceBase,
)
from tokenspeed_kernel.thirdparty.cute_dsl.mega_moe.token_comm import (
    CombineFormat,
    TokenCommArgs,
    TokenSrcMetadata,
)
...
@cute.jit
def cvt_f32_to_ue8m0_to_f32(fp32x1, loc=None, ip=None):
    """Round-trip fp32 -> ue8m0 -> fp32: the E8M0 decode-scale quantize step
    (rp/satfinite downcast, then upcast through bf16)."""
...
class QuantImpl:
    ...
    def __post_init__(self):
        ...
        elif self.quant_kind != "nvfp4":
            raise ValueError(
                f"quant_kind must be a CombineFormat or 'nvfp4', "
                f"got {self.quant_kind!r}."
            )
    ...
    def _data_rcp_limit(self) -> float:
        # 1 / max representable magnitude of the data element type. Only e2m1
        # and e4m3 data planes exist (orthodox nvfp4 + CombineFormat acts).
        dt = self.data_dtype
        if dt is cutlass.Float4E2M1FN:
            return Nvfp4E2M1RcpLimit  # 1/6
        assert dt is cutlass.Float8E4M3FN, f"unexpected data dtype {dt}"
    ...
    def mxfp8_combine_quant_regs_in_thread_impl(
        ...
        vec = self.sf_vec_size
        n_blocks = cute.size(prequant_reg) // vec
        data = cute.make_rmem_tensor((cute.size(prequant_reg),), cutlass.Float8E4M3FN)
    ...
    def _e8m0(
        ...
        scale_f32 = cutlass.Float32(cvt_f32_to_ue8m0_to_f32(candidate))
...
class SwapABSwigluFp4Epilogue:
    ...
    def run(
        ...
        tidx: cutlass.Int32,
        optional_epi_args: NvFp4OptinalEpiArgs = None,  # Epilogue optional runtime arguments.
    ...
    def alpha_swiglu_clamp(
        ...
        for i in cutlass.range_constexpr(0, n, 2):
            ...
            if cutlass.const_expr(self.situ_beta is None):
                ...
                gs = _sigmoid(
                    *cute.arch.mul_packed_f32x2((g0, g1), (inv_2beta, inv_2beta))
                )
                ...
                us = _sigmoid(
                    *cute.arch.mul_packed_f32x2((u0, u1), (inv_2lbeta, inv_2lbeta))
                )
...
class Fc2OutputRouter:
        ...
        for iter_idx in cutlass.range_constexpr(copy_iters):
            ...
            if token_valid and hidden_valid:
                ...
                if cutlass.const_expr(self.metadata is None):
                    ...
                    # byte_align mirrors get_data_dst: the STG vector is 16 B
                    # for e2m1 wires (odd warps at row base +16 B), 32 B for
                    # fp8/bf16.
                    dst_ptrs[iter_idx] = self.peer_rank_ptr_mapper.ptr_map_to_rank(
                        ...
                        byte_align=min(
                            32,
                            (
                                min(32, 256 // self.data_output.element_type.width)
                                * self.data_output.element_type.width
                            )
                            // 8,
                        ),
                    ).toint()
    ...
    def get_data_dst(
        ...
        # Alignment must match the real STG granularity: the store vector is
        # min(32, 256 // width) elements, i.e. 32 B for fp8/bf16 wires but
        # only 16 B for e2m1 (4-bit) -- odd warps then land on row base +16 B,
        # so an align-32 promise would be UB for the fp4 combine format.
        _stg_bytes = (
            min(32, 256 // self.data_output.element_type.width)
            * self.data_output.element_type.width
        ) // 8
        ptr = cute.make_ptr(
            ...
            addr,
            AddressSpace.gmem,
            assumed_align=min(32, _stg_bytes),
        )
        return ptr, pred
    ...
    def get_sf_dst(
        ...
        # natural element alignment (e8m0 1 B / bf16 2 B). An align-4 promise
        # here is UB: adjacent warps land on row base +1/+2/+3 for 1-byte
        # scales.
        sf_ptr = cute.make_ptr(
            self.sf_output.element_type,
            addr,
            AddressSpace.gmem,
            assumed_align=self.sf_output.element_type.width // 8,
        )
        return sf_ptr, pred
